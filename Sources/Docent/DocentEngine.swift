...
public struct DocentChunk: Identifiable {
    ...
    public init(id: Int64, title: String, text: String, sourceFile: String) {
        self.id = id
        self.title = title
        self.text = text
...
public class DocentEngine {
    ...
    public func query(_ text: String, topK: Int = 3) async throws -> [DocentResult] {
        ...
        for (chunkId, vector) in vectors {
            ...
            results.append(DocentResult(chunk: chunk, score: Double(score)))
        }
    ...
    private func loadAllChunks() throws -> [Int64: DocentChunk] {
        ...
        let sql = "SELECT id, title, content, file_path, encryption_type FROM docent_chunks;"
        ...
        while sqlite3_step(stmt) == SQLITE_ROW {
            ...
            let contentPtr = sqlite3_column_blob(stmt, 2)
            let contentLen = sqlite3_column_bytes(stmt, 2)
            ...
            chunks[id] = DocentChunk(id: id, title: title, text: content, sourceFile: filePath)
        }
        store.finalize(stmt)
        return chunks
    }
